...
<head>
  ...
  <title>Spectre Monotile Jigsaw Puzzle</title>
  <meta name="description" content="Interactive Spectre Aperiodic Monotile Jigsaw Puzzle SPA built with vanilla HTML5, CSS3, and JavaScript/SVG.">
  <style>
    :root {{
      --bg-color: #0f172a;
      --panel-bg: #1e293b;
...
<body>
  ...
  <div id="sidebar">
    <div class="header-box">
      ...
      <div class="subtitle">Aperiodic Jigsaw Puzzle SPA</div>
    </div>

    <!-- Game Flow Primary Toggle Button -->
    <button id="btn-game-toggle" class="btn-primary-action" title="Toggle between solved view and puzzle challenge mode">
      🎮 Start Game
    </button>
  ...
  <div id="help-modal" class="modal-overlay">
    <div class="modal-card">
      ...
      <ul>
        <li><strong>Start Game / Solution Toggle:</strong> Click <strong>Start Game</strong> to clear the solved layout and start solving. Toggle <strong>Show Solution</strong> anytime to check your progress.</li>
  ...
  <script>
    (function() {{
      ...
      // Exact 14-vertex Spectre monotile geometry (Smith et al. 2023)
      ...
      // 2. STATE MANAGEMENT
      const state = {{
        tiles: [], // Currently placed tiles on board: {{ id, x, y, rotationDeg, color }}
        nextTileId: 1000,
        selectedTileId: null,
        gameState: 'SOLVED_VIEW', // 'SOLVED_VIEW', 'PLAYING', 'SHOWING_SOLUTION'
      ...
      function validateTilePlacement(candVerts, ignoreTileId = null) {{
        ...
        // 2. Overlap check against all placed tiles
      ...
      // UI Game Flow Primary Toggle Button ("Start Game" / "Show Solution")
      function updateGameToggleButton() {{
        if (state.gameState === 'SOLVED_VIEW') {{
          btnGameToggle.textContent = '🎮 Start Game';
          btnGameToggle.title = 'Clear board and start puzzle challenge';
        }} else if (state.gameState === 'PLAYING') {{
          btnGameToggle.textContent = '💡 Show Solution';
          btnGameToggle.title = 'Toggle full solved layout onto board';
        }} else if (state.gameState === 'SHOWING_SOLUTION') {{
          btnGameToggle.textContent = '🙈 Hide Solution';
          btnGameToggle.title = 'Clear solution and resume solving';
        }}
      }}

      btnGameToggle.addEventListener('click', () => {{
        if (state.gameState === 'SOLVED_VIEW') {{
          // Start Game -> Clear solved tiles from board
          state.tiles = [];
          state.selectedTileId = null;
          state.gameState = 'PLAYING';
          renderPlacedTiles();
          updateGameToggleButton();
          showToast("🎮 Game Started! Drag tiles from palette into the frame.");
        }} else if (state.gameState === 'PLAYING') {{
          // Show Solution -> Load solved tiles
          state.tiles = JSON.parse(JSON.stringify(SOLVED_DELTA2_TILES));
          state.selectedTileId = null;
          state.gameState = 'SHOWING_SOLUTION';
          renderPlacedTiles();
          updateGameToggleButton();
          showToast("💡 Solved layout displayed!");
        }} else if (state.gameState === 'SHOWING_SOLUTION') {{
          // Hide Solution -> Clear board
          state.tiles = [];
          state.selectedTileId = null;
          state.gameState = 'PLAYING';
          renderPlacedTiles();
          updateGameToggleButton();
          showToast("🙈 Solution hidden. Ready to solve!");
        }}
      }});
      ...
      btnClear.addEventListener('click', () => {{
        if (state.tiles.length === 0 || confirm("Clear all tiles from board?")) {{
          state.tiles = [];
          state.selectedTileId = null;
          if (state.gameState === 'SOLVED_VIEW') state.gameState = 'PLAYING';
          updateGameToggleButton();
          renderPlacedTiles();
        }}
      }});
      ...
      // INITIALIZATION (Load initial solved state)
      function init() {{
        centerViewport();
        renderBoundaryFrame();
        renderPalette();

        // Load initial solved state on page load
        state.tiles = JSON.parse(JSON.stringify(SOLVED_DELTA2_TILES));
        state.gameState = 'SOLVED_VIEW';
        updateGameToggleButton();
        renderPlacedTiles();
        updateHUD();
      }}
...
print("Generated full SPA index.html successfully!")
