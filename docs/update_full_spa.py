...
<body>
  ...
  <script>
    (function() {{
      ...
      // 2. STATE MANAGEMENT (With In-Progress Board State Stashing)
      const state = {{
      ...
      // UPDATED GAME TOGGLE LISTENER (With In-Progress Board State Stashing)
      btnGameToggle.addEventListener('click', () => {{
        if (!state.isGameActive) {{
          if (state.savedUserTiles.length > 0) {{
            // Returning to an in-progress game from "Show Solution"
            ...
            // Starting a fresh game from initial solved load
            state.tiles = [];
            state.selectedTileId = null;
            state.isGameActive = true;
            btnGameToggle.textContent = '💡 Show Solution';
          ...
          // Player requested to see the solution while playing
          state.savedUserTiles = JSON.parse(JSON.stringify(state.tiles)); // Stash user tiles
      ...
      // INITIALIZATION (Load initial solved view)
      function init() {{
        centerViewport();
        renderBoundaryFrame();
        renderPalette();

        // Initial view shows full solved layout
        state.tiles = JSON.parse(JSON.stringify(state.solvedTiles));
        state.isGameActive = false;
...
print("Updated index.html with state stashing successfully!")
