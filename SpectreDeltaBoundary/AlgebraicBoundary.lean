...
lemma foldl_toStep_scale (w : List AllowedStep) (acc : Int) :
    w.foldl (fun a s => a + 30 * s.toStep) (30 * acc) = 30 * w.foldl (fun a s => a + s.toStep) acc := by
  induction w generalizing acc with
  | nil => rfl
  | cons hd tl ih =>
    dsimp
    have h_eq : 30 * acc + 30 * hd.toStep = 30 * (acc + hd.toStep) := by ring
    rw [h_eq]
    exact ih (acc + hd.toStep)

lemma boundaryToWalk_curvature_eq_sum_drop3 (w : BoundaryWord) :
    (boundaryToWalk w).curvature = ((w.drop 3).map stepToDegrees).sum := by
  induction w with
  | nil => rfl
  | cons a rest ih =>
    match rest with
    ...
    | b :: c :: rest' =>
      ...
      | [] => rfl
      | d :: rest'' =>
        have h_eq : (boundaryToWalk (a :: b :: c :: d :: rest'')).curvature =
                    stepToDegrees d + (boundaryToWalk (b :: c :: d :: rest'')).curvature := rfl
        rw [h_eq, ih]
        rfl
...
/-- Gets the vertex at index i along the path generated by boundary word w -/
def wordVertex (w : BoundaryWord) (i : Nat) : LatticePoint :=
  match (wordToPath w)[i]? with
  | some v => v
...
/-- Computes the forced tile orientation for a lock pattern at its exact boundary position.
...
noncomputable def lockForcedTile (w : BoundaryWord) (_hLock : ContainsLock w) : PlacedTile :=
  open Classical in
  if h : ∃ i, (w.drop i).take 3 = [.z0, .p60, .z0] then
    PlacedTile.mk (wordVertex w (Classical.choose h)) 0
  else if h : ∃ i, (w.drop i).take 3 = [.p60, .p90, .p60] then
    PlacedTile.mk (wordVertex w (Classical.choose h)) 2
  else if h : ∃ i, (w.drop i).take 4 = [.z0, .m60, .p90, .p60] then
    PlacedTile.mk (wordVertex w (Classical.choose h)) 3
  else if h : ∃ i, (w.drop i).take 4 = [.p60, .p90, .m60, .p90] then
    PlacedTile.mk (wordVertex w (Classical.choose h)) 0
  else if h : ∃ i, (w.drop i).take 4 = [.p90, .m60, .p90, .p60] then
    PlacedTile.mk (wordVertex w (Classical.choose h)) 8
  else
